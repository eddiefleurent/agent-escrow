...
13. **Delegation Capability Tokens (DCTs)** ✓ -- attenuated off-chain capability tokens based on Macaroons/Biscuits semantics, scoped to escrow + subject + operations/resources + expiry, with strict attenuation on delegation, introspection, explicit revoke, and automatic invalidation on escrow terminal states and emergency freeze/resolve hooks. Exposed across HTTP API, MCP tools, and `escrow-cli` commands (paper §6.1: restriction chaining across delegation chains).
   - **13a. Canonical token profile hardening** ✓ -- strict canonical profile `dct-profile-v1` only (no legacy token acceptance), deterministic caveat encoding (`op=...`, `res=...`, `exp<=...`), strict attenuation (must narrow at least one of ops/resources/expiry), full delegation-chain validation (lineage checks, cycle detection, ancestor validity requirements), and parity enforcement across MCP/HTTP/CLI.
   - **13b. Principal authorization layer for DCT operations** ✓ -- enforce who can mint/delegate/revoke/introspect by escrow role + trust context; default-deny policy engine (`internal/authz`), audited policy decisions (`dct_authorization_audit` table), emergency override path for factory owner, caller identity propagated across MCP/HTTP/CLI surfaces (paper §4.7, §6.1).
14. **Verifiable credentials for bidding** ✓ -- agents present signed capability attestations (`attestation-v1` profile) during sealed-bid reveal; delegator specifies `required_credentials_json` on RFQ with domain/capability/trusted-issuer selectors; server validates secp256k1 signatures, subject binding, expiry, and requirement matching; `accept_bid` gated on `credential_verified` when requirements present; Bazaar-compatible discovery metadata at `GET /api/v1/bazaar/discovery`; forward-compatible `issuer_did` field for V4 DID integration (paper §4.6 Table 3: Web of Trust with DIDs)
15. **Attestation chains** ✓ -- recursive verification across delegation chains (A -> B -> C); each link produces a signed `completion-attestation-v1` attestation of sub-task completion; parent-child escrow linkage (`parent_escrow_id` on RFQs and escrows); off-chain DAG validation (cycle detection, parent-link integrity, child escrow coverage, signature verification via secp256k1 recovery); attestation chain storage with root hash and verification summary; exposed across HTTP API (`GET /api/v1/escrows/{id}/attestation-chain`, `GET /api/v1/escrows/{id}/children`, attestation-enriched `GET /api/v1/escrows/{id}`), MCP tools (`get_attestation_chain`, `submit_work` with `attestation_chain_json`), and `escrow-cli` commands (`attestation-chain`, `children`); buyer-must-be-worker constraint enforced on sub-delegation RFQ creation (paper §4.8: transitive liability, chain of custody)
16. **Checkpoint/resume** ✓ -- standardized checkpoint artifacts for mid-task agent swaps; active worker commits `state_snapshot` URIs to shared storage with optional milestone scoping, content hashing, and completion percentage; replacement workers retrieve latest checkpoint via MCP tools (`commit_checkpoint`, `list_checkpoints`, `get_latest_checkpoint`), HTTP API (`POST/GET /api/v1/escrows/{id}/checkpoints`, `GET .../checkpoints/latest`), and `escrow-cli` commands (`checkpoint-commit`, `checkpoints`, `checkpoint-latest`); authorization enforced (active worker only); off-chain event published on commit; couples with existing V2 milestone settlement for partial compensation (paper §6.1: checkpoint artifacts + partial compensation clauses; §4.4: checkpoint-based re-allocation)
17. **Tiered service levels** ✓ -- low-assurance (optimistic, tier 0) vs high-assurance (verified, tier 1) delegation paths with per-tier protocol fee snapshots and differentiated approval rules; `serviceTier` stored immutably on-chain; high-assurance escrows block `approveByBuyer` / `approveMilestoneByBuyer` (verifier-only settlement); `highAssuranceFeeBps` set at factory level and snapshotted into each escrow at creation; propagated through Go chain layer, storage, bidding, API, MCP tools, and event indexer (paper §5.3: ensure safety does not become a luxury good)
...
21. **Contract-first decomposition tooling** ✓ -- decomposition planning and validation flow across MCP tools, HTTP API, and `escrow-cli`; structural validation for leaf-node verifiability (`optimistic`, `quorum`, `zk_proof`, `unit_test`), per-leaf market context signals (`proven`/`emerging`/`untested`) that inform but do not gate, optional leaf-level `delegate_preference` metadata (`human`/`ai`/`any`) to explicitly mark hybrid human/AI allocation intent without hard gating, and decomposition finalization that creates RFQs with verification-type-aware service tiers and bidding modes (paper §4.1: decompose until units match formal proofs or automated tests)
...
**V3**: Contract-first decomposition tooling -- recursive task decomposition until leaf sub-tasks are structurally verifiable (formal proofs, quorum, automated tests, or optimistic review), with market-depth context surfaced for caller judgment, advisory per-leaf human/AI delegate preference signaling, and finalize-time RFQ generation for market assignment.
...
**V3**: Checkpoint state snapshots for mid-task agent swaps. Market stability mechanisms implemented: parent-linked re-bid cooldown, damped reputation overlays on immutable raw counters, and on-chain re-delegation surcharge snapshots.
...
**V3**: Tiered service levels ensure minimum viable reliability for all participants. Contract-first decomposition adds advisory leaf-level human/AI allocation intent metadata for hybrid market planning.

**V4**: Curriculum-aware task routing for human skill preservation. Cognitive friction calibration with context-aware intensity. Liability firebreaks in long chains. Insurance/liability frameworks. Governance-enforced safety floors.
