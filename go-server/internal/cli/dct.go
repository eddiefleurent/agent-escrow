...
func newDCTCmd(opts *Options) *cobra.Command {
	...
	dctCmd.AddCommand(newDCTPostCmd(opts, "emergency-override", "Emergency override for DCT operations (owner only)", "/api/v1/dcts/emergency-override", true))
	dctCmd.AddCommand(newDCTEscrowListCmd(opts))
	dctCmd.AddCommand(newDCTAuditCmd(opts))
	return dctCmd
}
...
func newDCTAuditCmd(opts *Options) *cobra.Command {
	return &cobra.Command{
		Use:   "audit [escrow-id]",
		Short: "List DCT authorization audit log entries",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) > 0 {
				...
				params["escrow_id"] = strconv.FormatInt(escrowID, 10)
			}
			return runGet(cmd, opts, "/api/v1/dcts/audit", params)
		},
	}
}
